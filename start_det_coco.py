...
def parse_coco_annotations(coco_file):
    ...
    CatInfo = namedtuple("CatInfo", ["name", "id"])
    AnnoInfo = namedtuple("AnnoInfo", ["bbox", "id", "category"])
    ImageInfo = namedtuple("ImageInfo", ["filename", "id", "width", "height", "annotations"])

    images = { it["id"]: ImageInfo(filename=it["file_name"],
                                   id=it["id"],
                                   width=it["width"],
                                   height=it["height"],
                                   annotations=[]) for it in coco["images"]}
    categories = { it["id"]: CatInfo(name=it["name"],
                                     id=it["id"]) for it in coco["categories"]}
    for it in coco["annotations"]:
        images[it["image_id"]].annotations.append(
            AnnoInfo(bbox=it["bbox"], id=it["id"], category=categories[it["category_id"]])
        )
...
def prepare_dataset_3(ssplits=["train", "test", "validation"], dsplits=["train", "test", "val"]):
    ...
    for s, t in zip(ssplits, dsplits):
        ...
        coco_file = os.path.join(f"/dataset/{s}/annotations", os.listdir(f"/dataset/{s}/annotations")[0])
        cocodata = parse_coco_annotations(coco_file)
